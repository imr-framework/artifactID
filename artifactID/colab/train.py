...
for artifact_folder in data_root.glob('*'):
    ...
    label = artifact_folder.name.split('_')[0]
...
def train_generator(batch_size):
    ...
    while True:
        ...
        for i in range(batch_size):
            ...
            if 'b0_' in str(x_paths_train[counter]):  # Orient B0 data correctly, was not done during datagen
                vol = np.rot90(vol, -1, axes=(0, 1))
            x.append(vol)
            label = y_labels_train[counter]  # Get label
            y.append(dict_labels_encoded[label])  # Add encoded label
            counter += 1
            if counter > len(x_paths_train):  # This means one/another epoch is done
...
#                                          output_shapes=(
#                                          tf.TensorShape([None, 240, 240, 155, 1]), tf.TensorShape([1])))
...
batch_size = 1
steps_per_epoch = math.ceil(len(x_paths_train) / batch_size)
# model.fit(dataset, steps_per_epoch=steps_per_epoch)
model.fit(x=train_generator(batch_size=batch_size), steps_per_epoch=steps_per_epoch)
