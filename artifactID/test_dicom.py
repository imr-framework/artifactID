...
def main(path_read_data: str, path_model_root: str, patch_size: int, viz: bool = True):
    # =========
    # DATA LOADING
    # =========
    ...
    arr_folders = list(map(lambda getparent: getparent.parent, arr_files))  # Get parent folder of each file
    arr_folders = np.unique(arr_folders)  # Get unique folders

    # Convert DICOM to npy
    print('Reading DICOMs...')
    arr_vols = []
    for f in tqdm(arr_folders):
        npy = data_ops.dcmfolder2npy(path=f, verbose=False)
        if npy is not None:  # Avoid edge case: different sized images in same folder
            arr_vols.append(npy)
