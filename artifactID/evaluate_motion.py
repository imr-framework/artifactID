...
def main(path_model: Path, path_txt: Path):
    ...
    for path in path_eval_npy:
        ...
        if path.parent.name.lower() in ['gibbs', 'wrap', 'motion']:
            arr_labels.append(1)
        ...
        npy = np.rot90(npy, 1)  # Fix orientation
        npy = cv2.resize(npy, dsize=(256, 256))  # Resize to 256, 256
        npy = (npy - npy.mean()) / npy.std()  # Zero mean, unit std
        arr_npy.append(npy)
    arr_npy = np.expand_dims(arr_npy, -1)

    # Inference
    arr_y_pred = model.predict(x=arr_npy)
    arr_y_pred = np.argmax(arr_y_pred, axis=-1)

    arr_labels = np.array(arr_labels)
    arr_ypred = np.array(arr_y_pred)
...
if __name__ == '__main__':
    path_root = Path(r"E:\Data\Source\ArtifactID\ArtifactID_motion")
    path_txt = path_root / 'val.txt'
    path_model = Path(r"output\2021")
